      ...
      <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20 select-none overflow-x-hidden font-black">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1 leading-none text-[9px]">
                  ...
                  <p className="text-white/50 tracking-widest uppercase ml-1">v17.50 master</p>
                  {userRole === 'master' && (
                ...
                <>
                  <div className="bg-white p-6 rounded-3xl border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm text-slate-800">
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto leading-none">
                      ...
                      {userRole === 'master' && <button onClick={handleBulkAttendanceToggle} className="flex items-center gap-2 bg-emerald-500 text-white px-5 py-3 rounded-2xl font-black shadow-lg hover:bg-emerald-600 transition active:scale-95 whitespace-nowrap leading-none"><CheckCircle size={18} /> 일괄 출석</button>}
                    </div>
                  </div>
                        ...
                        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                          ...
                          <div className="flex gap-2">
                            <BufferedInput value={newPlan.unit} onSave={(v) => setNewPlan({ ...newPlan, unit: v })}
                              placeholder="단원 / 내용 입력..." className="flex-1 px-4 py-2.5 rounded-2xl border bg-white font-bold text-sm outline-none focus:border-teal-400 transition-all shadow-sm" />
                            <BufferedInput value={newPlan.memo} onSave={(v) => setNewPlan({ ...newPlan, memo: v })}
                              placeholder="메모 (선택)" className="flex-1 px-4 py-2.5 rounded-2xl border bg-white font-medium text-sm outline-none focus:border-teal-400 transition-all shadow-sm" />
                            <button onClick={addPlan} className="px-5 py-2.5 bg-teal-600 text-white rounded-2xl font-black text-sm shadow-md hover:bg-teal-700 transition-all active:scale-95 leading-none">추가</button>
                          </div>
                        </div>
                      )}
                                ...
                                <div className="space-y-3 animate-in slide-in-from-top-2">
                                  ...
                                  <div className="flex gap-2">
                                    <BufferedInput value={editPlanData.unit} onSave={(v) => setEditPlanData({ ...editPlanData, unit: v })}
                                      placeholder="단원 / 내용" className="flex-1 px-3 py-2 border rounded-xl font-bold text-sm bg-slate-50 outline-none focus:border-teal-400 shadow-sm" />
                                    <BufferedInput value={editPlanData.memo} onSave={(v) => setEditPlanData({ ...editPlanData, memo: v })}
                                      placeholder="메모" className="flex-1 px-3 py-2 border rounded-xl font-medium text-sm bg-slate-50 outline-none focus:border-teal-400 shadow-sm" />
                                  </div>
                                  <div className="flex gap-2">
                ...
                return (
                  <div className="max-w-2xl mx-auto space-y-6">
                    ...
                    {sortedDates.length === 0 ? (
                      ...
                      <div key={date} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden text-left">
            ...
            <div className="max-w-4xl mx-auto space-y-6 text-left">
              {userRole === 'master' && (
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 text-left">
                  ...
                  <BufferedTextarea value={bulkStudentInput} onSave={setBulkStudentInput} placeholder="이름을 입력하세요..." className="w-full h-32 px-4 py-4 rounded-2xl border bg-slate-50 font-bold resize-none mb-4 outline-none focus:bg-white shadow-inner" />
                  <button onClick={() => {
                    if (!bulkStudentInput.trim()) return;
                    const names = bulkStudentInput.split(/[,\n]/).map(n => n.trim()).filter(Boolean);
                    const batch = writeBatch(db);
                    names.forEach(name => {
                      const id = 's' + (crypto.randomUUID ? crypto.randomUUID().replace(/-/g, '') : Date.now() + Math.random().toString(36).substr(2, 9));
                  ...
                  }} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-lg shadow-xl hover:bg-indigo-700 transition-all leading-none">등록하기</button>
                </div>
              )}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {students.map(s => (
                  <div key={s.id} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm group transition-all text-left">
                    {editStudentId === s.id ? (
                      <div className="space-y-4 animate-in slide-in-from-top-2">
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1"><label className="text-[10px] text-slate-400 font-black">이름</label><input value={editStudentData.name} onChange={(e) => setEditStudentData({ ...editStudentData, name: e.target.value })} className="w-full px-3 py-2 border rounded-xl font-bold text-sm bg-slate-50 outline-none focus:border-indigo-500" /></div>
                          <div className="space-y-1"><label className="text-[10px] text-slate-400 font-black">코드</label><input value={editStudentData.studentCode} onChange={(e) => setEditStudentData({ ...editStudentData, studentCode: e.target.value })} className="w-full px-3 py-2 border rounded-xl font-bold text-sm bg-slate-50 outline-none focus:border-indigo-500" /></div>
                          <div className="space-y-1"><label className="text-[10px] text-slate-400 font-black">담임</label><input value={editStudentData.homeroomTeacher} onChange={(e) => setEditStudentData({ ...editStudentData, homeroomTeacher: e.target.value })} className="w-full px-3 py-2 border rounded-xl font-bold text-sm bg-slate-50 outline-none focus:border-indigo-500" /></div>
                          <div className="space-y-1"><label className="text-[10px] text-slate-400 font-black">고교</label><input value={editStudentData.highSchool} onChange={(e) => setEditStudentData({ ...editStudentData, highSchool: e.target.value })} className="w-full px-3 py-2 border rounded-xl font-bold text-sm bg-slate-50 outline-none focus:border-indigo-500" /></div>
                        </div>
                        <div className="flex gap-2 pt-2"><button onClick={saveStudentDetails} className="flex-1 py-2 bg-green-600 text-white rounded-xl font-black text-xs shadow-md transition-all">저장</button><button onClick={() => setEditStudentId(null)} className="px-4 py-2 bg-slate-100 text-slate-500 rounded-xl font-bold text-xs">취소</button></div>
                      </div>
                    ) : (
                      <div className="flex justify-between items-start">
                        <div className="space-y-3 flex-1">
                          <div className="flex items-center gap-2"><span className="font-bold text-xl text-slate-800">#{s.studentCode || '000'} {s.name}</span></div>
                          <div className="grid grid-cols-2 gap-2 text-slate-500 text-sm font-bold">
                            <div className="flex items-center gap-1.5"><UserCog size={14} /><span>{s.homeroomTeacher || "-"}</span></div>
                            <div className="flex items-center gap-1.5"><GraduationCap size={14} /><span>{s.highSchool || "-"}</span></div>
                          </div>
                        </div>
                        {userRole === 'master' && (
                          <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                            <button onClick={() => { setEditStudentId(s.id); setEditStudentData({ name: s.name, studentCode: s.studentCode || '', homeroomTeacher: s.homeroomTeacher || '', highSchool: s.highSchool || '' }); }} className="p-2 text-indigo-500 bg-indigo-50 rounded-xl"><Edit2 size={18} /></button>
                            <button onClick={() => deleteItem('students', s.id)} className="p-2 text-red-500 bg-red-50 rounded-xl"><Trash2 size={18} /></button>
                          </div>
                        )}
                      </div>
                    )}
            ...
            <div className="max-w-4xl mx-auto space-y-6 text-left">
              <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  <div className="space-y-1.5">
                    ...
                    <input type="date" value={reportRange.from} onChange={e => setReportRange(r => ({...r, from: e.target.value}))}
                      className="w-full px-4 py-3 rounded-2xl border bg-slate-50 font-bold outline-none focus:border-slate-400 transition-all shadow-sm" />
                  </div>
                  <div className="space-y-1.5">
                ...
                <button onClick={generateReport}
                  className="w-full py-4 text-white rounded-2xl font-black text-base shadow-lg flex items-center justify-center gap-2"
                ...
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                  <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <button onClick={() => { navigator.clipboard?.writeText(reportText); }}
                      className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl font-bold text-xs">
                      <Copy size={13} /> 복사
                    </button>
                  </div>
              ...
              {reportGenerated && (
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  {!aiAnalysis && !aiLoading && (
                    <div className="p-8 text-center">
                      ...
                      <button onClick={requestAiAnalysis}
                        className="px-8 py-4 text-white rounded-2xl font-black shadow-lg flex items-center gap-2 mx-auto"
                        style={{background:'var(--sc)'}}>
                  ...
                  {aiLoading && (
                    <div className="p-12 text-center text-slate-500 font-bold">분석 중...</div>
                  )}
                  {aiAnalysis && !aiLoading && (
                    <div className="p-6">
                ...
                <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
                  ...
                  <div className="space-y-8 text-slate-700">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div><p className="text-[10px] font-black text-slate-400 mb-3 uppercase flex items-center gap-1"><Tag size={12} /> 1. 과목</p><div className="flex flex-wrap gap-2">{SUBJECTS.map(sub => (<button key={sub} onClick={() => setNewAssignment({ ...newAssignment, subject: sub })} className={`px-3 py-1.5 rounded-xl text-xs font-bold border-2 transition ${newAssignment.subject === sub ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}>{sub}</button>))}</div></div>
                      <div><p className="text-[10px] font-black text-slate-400 mb-3 uppercase flex items-center gap-1"><TrendingUp size={12} /> 2. 수준</p><div className="flex flex-wrap gap-2">{ASSIGNMENT_LEVELS.map(lvl => (<button key={lvl} onClick={() => setNewAssignment({ ...newAssignment, level: lvl })} className={`px-3 py-1.5 rounded-xl text-xs font-bold border-2 transition ${newAssignment.level === lvl ? 'bg-indigo-700 border-indigo-700 text-white shadow-md' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}>{lvl}</button>))}</div></div>
                      <div>
                        <p className="text-[10px] font-black text-slate-400 mb-3 uppercase flex items-center gap-1"><UserCheck size={12} /> 3. 대상</p>
                        <div className="flex gap-2 mb-3">
                          <button onClick={() => setNewAssignment({ ...newAssignment, type: 'all', targetStudents: [] })} className={`flex-1 py-1.5 rounded-xl text-xs font-bold border-2 ${newAssignment.type === 'all' ? 'bg-indigo-600 border-indigo-600 text-white shadow-sm' : 'bg-white border-slate-100 text-slate-400'}`}>전체</button>
                          <button onClick={() => setNewAssignment({ ...newAssignment, type: 'individual' })} className={`flex-1 py-1.5 rounded-xl text-xs font-black border-2 ${newAssignment.type === 'individual' ? 'bg-indigo-600 border-indigo-600 text-white shadow-sm' : 'bg-white border-slate-100 text-slate-400'}`}>개별</button>
                        </div>
                        {newAssignment.type === 'individual' && (
                          <div className="p-3 bg-slate-50 rounded-2xl border max-h-[100px] overflow-y-auto no-scrollbar shadow-inner">
                            {students.map(s => (
                              <label key={s.id} className="flex items-center gap-1 text-[11px] font-bold text-slate-500 cursor-pointer hover:text-indigo-600">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div><p className="text-[10px] font-black text-slate-400 mb-3 uppercase flex items-center gap-1"><FileText size={12} /> 4. 명칭</p><BufferedInput value={newAssignment.title} onSave={(v) => setNewAssignment({ ...newAssignment, title: v })} placeholder="제목..." className="w-full px-4 py-3 rounded-2xl border bg-slate-50 font-bold outline-none focus:ring-2 focus:ring-indigo-100 shadow-sm" /></div>
                      <div><p className="text-[10px] font-black text-slate-400 mb-3 uppercase flex items-center gap-1"><Calendar size={12} /> 5. 마감기한</p><input type="date" value={newAssignment.deadline} onChange={(e) => setNewAssignment({ ...newAssignment, deadline: e.target.value })} className="w-full px-4 py-3 rounded-2xl border bg-slate-50 font-bold outline-none focus:border-indigo-500 shadow-sm" /></div>
                    </div>
                    <button onClick={addAssignment} className={`w-full ${regCategory === 'assignment' ? 'bg-indigo-600 shadow-indigo-100' : 'bg-purple-600 shadow-purple-100'} text-white py-4 rounded-2xl font-black text-lg shadow-xl hover:scale-[1.01] transition-all`}>발행하기</button>
                  </div>
                </div>
              )}
                    ...
                    {editItemId === a.id ? (
                      <div className="space-y-5 animate-in slide-in-from-top-2">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <BufferedInput value={editItemData.title} onSave={(v) => setEditItemData({ ...editItemData, title: v })} className="w-full px-2 py-1.5 border rounded-lg text-xs font-bold mb-2 focus:border-indigo-500 shadow-sm" />
                            <input type="date" value={editItemData.deadline || ''} onChange={(e) => setEditItemData({ ...editItemData, deadline: e.target.value })} className="w-full px-2 py-1.5 border rounded-lg text-xs font-bold shadow-sm" />
                          </div>
                        </div>
                        <div className="space-y-2">
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => setEditItemData({ ...editItemData, type: 'all', targetStudents: [] })} className={`px-4 py-1.5 rounded-lg text-xs font-bold border-2 ${editItemData.type === 'all' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white border-slate-200 text-slate-400'}`}>전체 학생</button>
                            <button onClick={() => setEditItemData({ ...editItemData, type: 'individual', targetStudents: editItemData.targetStudents || [] })} className={`px-4 py-1.5 rounded-lg text-xs font-bold border-2 ${editItemData.type === 'individual' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white border-slate-200 text-slate-400'}`}>개별 지정</button>
                          </div>
                          {editItemData.type === 'individual' && (
                            <div className="mt-2 p-4 bg-slate-50 rounded-2xl border border-slate-200 shadow-inner grid grid-cols-2 md:grid-cols-3 gap-2">
                              {students.map(s => {
                                const isChecked = (editItemData.targetStudents || []).includes(s.id);
                                return (
                                  <label key={s.id} className={`flex items-center gap-2 px-3 py-2 rounded-xl cursor-pointer border-2 transition-all text-xs font-bold ${isChecked ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm' : 'bg-white text-slate-500 border-slate-200'}`}>
                        ...
                        <div className="flex gap-2 pt-1"><button onClick={saveEditItem} className="flex-1 py-2 bg-green-600 text-white rounded-xl font-black text-sm flex items-center justify-center gap-2 shadow-lg"><Save size={16} /> 저장</button><button onClick={() => { setEditItemId(null); setEditItemData(null); }} className="px-6 py-2 bg-slate-100 text-slate-500 rounded-xl font-bold text-sm">취소</button></div>
                      </div>
                    ) : (
                      <div className="flex justify-between items-center p-1">
                        <div className="flex flex-col gap-1">
                          ...
                          <span className="font-black text-slate-800 text-lg leading-tight mt-0.5">{a.title}</span>
                          {a.deadline && <span className="text-[11px] text-slate-400 font-medium flex items-center gap-1 italic">{a.deadline} 마감</span>}
                        </div>
                        {userRole === 'master' && (
                          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                            <button onClick={() => { setEditItemId(a.id); setEditItemData({ ...a }); }} className="p-2.5 text-indigo-500 bg-indigo-50 rounded-xl"><Edit2 size={18} /></button>
                            <button onClick={() => deleteItem(regCategory === 'assignment' ? 'assignments' : 'memoItems', a.id)} className="p-2.5 text-red-500 bg-red-50 rounded-xl"><Trash2 size={18} /></button>
                          </div>
                        )}
                      </div>
                    )}
        ...
        {selectedTest && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden">
              <div className="bg-orange-600 p-8 text-white flex justify-between items-start">
                <div className="flex items-center gap-4 text-left">
                  <div className="w-14 h-14 bg-white/20 rounded-3xl flex items-center justify-center">{isTestEditMode ? <Edit2 size={32} /> : <FileSearch size={32} />}</div>
              ...
              <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto text-left">
                {isTestEditMode ? (
                  <div className="space-y-4">
                    ...
                    <BufferedInput value={selectedTest.source} onSave={(v) => setSelectedTest({ ...selectedTest, source: v })} placeholder="출처 입력..." className="w-full px-4 py-3 border-2 border-slate-100 rounded-2xl font-bold bg-white outline-none" />
                    <BufferedTextarea value={selectedTest.description} onSave={(v) => setSelectedTest({ ...selectedTest, description: v })} className="w-full h-40 p-5 border-2 border-slate-100 rounded-3xl font-medium text-sm bg-white outline-none" />
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-3">
                      <div className="flex flex-col items-center gap-1.5 bg-slate-50 border border-slate-100 rounded-2xl py-4">
                        <Calendar size={16} className="text-orange-400" />
                        <p className="text-[9px] font-black text-slate-400 uppercase">시험 날짜</p>
                        <p className="text-sm font-black text-slate-800">{selectedTest.date || '-'}</p>
                      </div>
                      <div className="flex flex-col items-center gap-1.5 bg-slate-50 border border-slate-100 rounded-2xl py-4">
                        <Tag size={16} className="text-orange-400" />
                        <p className="text-[9px] font-black text-slate-400 uppercase">난이도</p>
                        <p className="text-sm font-black text-slate-800">{selectedTest.difficulty || '-'}</p>
                      </div>
                      <div className="flex flex-col items-center gap-1.5 bg-slate-50 border border-slate-100 rounded-2xl py-4">
                        <Bookmark size={16} className="text-orange-400" />
                        <p className="text-[9px] font-black text-slate-400 uppercase">출처</p>
                        <p className="text-sm font-black text-slate-800">{selectedTest.source || '-'}</p>
                      </div>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-200 pb-3">
                      <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest flex items-center gap-1"><ClipboardList size={14} /> 상세 범위</p>
                      {userRole === 'master' && <button onClick={() => setIsTestEditMode(true)} className="text-xs font-bold text-indigo-600">수정</button>}
                    </div>
                ...
                {userRole === 'master' ? (
                  <div className="space-y-1.5">
                    ...
                    <BufferedTextarea
                      ...
                      className="w-full h-24 px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium text-slate-700 outline-none resize-none select-text shadow-inner"
                    />
                  </div>
                  ...
                  <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 text-left">
                    ...
                    <p className="text-sm text-slate-600 font-medium whitespace-pre-wrap">{studentNotes[selectedStudent.id]}</p>
                  </div>
                )}
        ...
        {statusMenu && (
          <div className="fixed inset-0 z-[150]" onClick={() => setStatusMenu(null)}>
            ...
            >
              <p className="px-3 py-1 text-[9px] font-black text-slate-400 uppercase border-b border-slate-100 mb-1">상태 변경</p>
        ...
        {bulkDatePopup && (
          <div className="fixed inset-0 z-[160] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in" onClick={() => { setBulkDatePopup(null); setBulkSelectedStatus(null); }}>
            <div className="bg-white rounded-[2rem] shadow-2xl p-8 w-full max-w-xs animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
              ...
              <input type="date" value={bulkSelectedDate} onChange={e => setBulkSelectedDate(e.target.value)}
                className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-slate-800 outline-none focus:border-indigo-400 mb-6 text-center" />
                ...
                {(bulkDatePopup.category === 'assignment' ? ASSIGN_STATUS_ORDER : MEMO_STATUS_ORDER).map(st => {
                  ...
                  return (
                    <button key={st} onClick={() => setBulkSelectedStatus(st)}
                      className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs font-black transition-all border-2 ${isSelected ? `${cfg?.bg} ${cfg?.color} border-current ring-2 ring-current` : `${cfg?.bg} ${cfg?.color} border-transparent opacity-60 hover:opacity-100`}`}>
                      {cfg?.icon && React.createElement(cfg.icon, { size: 14 })}
                      {cfg?.label}
                    </button>
                  );
              ...
              <div className="flex gap-3 mt-2">
                <button onClick={() => { setBulkDatePopup(null); setBulkSelectedStatus(null); }} className="flex-1 py-3 bg-slate-100 text-slate-400 rounded-2xl font-black text-sm">취소</button>
                <button onClick={() => { if (bulkSelectedStatus) { bulkUpdateStatus(bulkDatePopup.item, bulkSelectedStatus, bulkDatePopup.category); setBulkSelectedStatus(null); } }}
                  disabled={!bulkSelectedStatus} className={`flex-1 py-3 rounded-2xl font-black text-sm transition ${bulkSelectedStatus ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-slate-100 text-slate-300'}`}>완료</button>
              </div>
            </div>
          </div>
        )}
